...
void updateState(){
    ...
    // Get sensor data
  float height_k = getRelAltitude();  
  float acc_k = getRelAccelX(); //it is absolute acceleration! (not relative!)
...
Matrix<1,3> H_k = { 1    ,  0  , 0    };

// input from IMU for process prediction
float u_k= -acc_k - g*cos(theta_k);

//prediction step (prior)
Matrix<3> xk_minus =  F_k*previousState + G_k*u_k ;               
Matrix<3,3> Pk_minus = F_k * previousVariance *(~F_k)+Q;

// Kalman gain
Matrix<3,1>H_k_T=~H_k;
//Matrix<1> S_k = (H_k * Pk_minus * H_k_T)+R;
float S_k = Pk_minus(1,1)+R;
Matrix<3> K_k = Pk_minus * H_k_T/S_k;

// weight to previous estimates
Matrix<3,3> I3 = {1,0,0,0,1,0,0,0,1};
Matrix<3,3> W_k=I3-K_k*H_k;

//update step (posterior)
Matrix<3>x_k = W_k*xk_minus+K_k*height_k;
Matrix<3,3>P_k = W_k*Pk_minus*(~W_k)+K_k*R*(~K_k);
