...
void loop() {
  ...
  //generating simulated height data
  //by the time this is encountered for the first time, it’s already probably 8-11 seconds
  timeSinceLiftOff = timeNow-20.0f; //assuming 10 seconds of idleness before liftoff
  currentHeight = 40.0f*timeSinceLiftOff; //assuming average velocity
  ...
  if(icmReady && bmpReady){
    ...
    if(icm.getEvent(&accel, &gyro, &temp, &mag)){   
      ...
      if(!isLaunched && currentHeight > 10.0f){
  ...
  if (bmpReady && icmReady && isLaunched) {
    ...
    if (!isActuating && currentHeight >= ACTUATION_HEIGHT && isLaunched && (timeNow - launchTime) >= 1.0f){
    //if (!isActuating && (currentHeight >= ACTUATION_HEIGHT){
      desiredActuation = 540.0f;
      isActuating = true;
  ...
  if(!isFRAMDumped && isLaunched && (timeNow - launchTime >= 25.0f)){ // record 100 seconds at launch
    isFRAMDumped = true;
    framDumpToSD();
    soundBuzzer(1);
  }
